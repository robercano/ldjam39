...
public class PlayerHealth : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        playerFistAttack = GetComponent<PlayerFistAttack>();

        timeFromLastKnockOut = -invulnerabilityTime;
    }
    ...
    {
        if (IsInvulnerable() && IsKnockedOut() == false)
        {
            float alpha = invulnerabilityAlpha + (1.0f - invulnerabilityAlpha) * Mathf.Sin(20.0f * Mathf.PI * (Time.time - timeFromLastKnockOut) / invulnerabilityTime);

            spriteRenderer.color = new Color(1f, 1f, 1f, alpha);

            playerFistAttack.SetFistAlpha(alpha);
        }
        else
        {
            if (spriteRenderer.color.a != 1f)
            {
                ...
                playerFistAttack.SetFistAlpha(1.0f);
            }
        }
    }
